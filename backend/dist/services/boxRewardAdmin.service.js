...
exports.createBoxReward = createBoxReward;
exports.updateBoxReward = updateBoxReward;
exports.deleteBoxReward = deleteBoxReward;
exports.listBoxRewardsByBox = listBoxRewardsByBox;
const validation_1 = require("./validation");
const adminAuditLog_service_1 = require("./adminAuditLog.service");
// Helper to compute expected value of rewards
function computeExpectedValue(rewards, boxPrice) {
...
async function createBoxReward(data, tx) {
    (0, validation_1.validateBoxRewardInput)(data);
    ...
    await (0, adminAuditLog_service_1.logAdminAction)('create', 'BoxReward', reward.id, null, reward, tx);
    return filterBoxReward(reward);
}
async function updateBoxReward(id, data, tx) {
    (0, validation_1.validateBoxRewardInput)(data);
    ...
    await (0, adminAuditLog_service_1.logAdminAction)('update', 'BoxReward', id, oldReward, reward, tx);
    return filterBoxReward(reward);
}
async function deleteBoxReward(id, tx) {
    ...
    await (0, adminAuditLog_service_1.logAdminAction)('delete', 'BoxReward', id, reward, null, tx);
    return { success: true };
}
async function listBoxRewardsByBox(boxId, tx) {
    const rewards = await tx.boxReward.findMany({ where: { boxId } });
    return rewards.map(filterBoxReward);
}
function filterBoxReward(reward) {
    return {
        id: reward.id,
        boxId: reward.boxId,
